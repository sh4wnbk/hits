...
| Sub-task 0 | Created this BOB_USAGE.md stub | BOB_USAGE.md |
...
| Sub-task 0 | Claude Code | Phase 1/2 tool handoff recorded | BOB_USAGE.md |
...
| Prompt restructure, one bounded attempt | Claude Code | Two changes. The `n_a` leak closed at source: `permitted_numbers()` omits the frame clause for a frameless entry rather than printing the schema's placeholder, with a test asserting the token is absent from the system turn, the user turn and a regeneration turn across all three objects. Then the standing rules moved from the user turn into the system turn, `generate()` threaded to take one, plus an instruction to open with a plain-language sentence. Measured across three objects, five runs each: 0 of 15 grounded, every one to the floor. A controlled A/B with the rules back in the user turn and the `n_a` fix retained gave 4 of 5 grounded, so the restructure is what regressed it. The new failure mode is typography, `km² s⁻²` for `km^2/s^2` and U+2011 non-breaking hyphens inside ISO dates, which is what asking a model for plain prose does to a model also told to quote exactly. Stopped at the agreed hard stop rather than iterating on wording. Verdict: floor-first | agent/explain.py, agent/granite.py, tests/test_explain.py |
| Gate limit found live | Claude Code | A grounded 3I/ATLAS answer contained "September 20, 2030", a reformatted date the prompt forbids and the gate accepted, because `20` is `solve.tof_years` and `2030` is `solve.departure.year` in that manifest. Membership cannot enforce the date rule when a date's fragments are independently grounded. Recorded, not fixed; it belongs with the accepted limits in `tests/corpus/known_limits.jsonl` | CLAUDE.md (finding only) |

| Grounded form made the human form | Claude Code | Three changes to the two strict components, each adding exact forms the solver emits and never loosening a match. verify/normalize.py canonicalizes typography before matching (Unicode dashes, superscripts, a closed list of unit-string rewrites), asserted unable to alter any numeral. Word forms for every unit, moved to solver/manifest.py so the manifest owns what a number may be written as and the gate only checks; the unit lookahead is derived from the table rather than pinned. Dates render in a closed set of two, ISO and one human form, and a date is now a single token, which closes the recorded fragmentation limit: "March 20, 2030" is one fabricated token instead of two accidentally-grounded digits. Recognition of date shapes is deliberately wider than acceptance, which only ever makes the gate stricter. Frozen fixture regenerated, diff purely additive, 81 entries either side. Suite 240 passed with the corpus, the pinned ladders and test_gate_does_no_arithmetic green | verify/normalize.py, verify/corpus_ingest.py, verify/exemptions.py, verify/extract.py, verify/groundedness.py, solver/manifest.py, agent/explain.py, docs/MANIFEST.md, tests/ |
| Three-object measurement, and a quota wall | Claude Code | Five live runs per object on the same restructured prompt that had scored 0/15 before these changes: 1I/'Oumuamua 5/5 `granite_after_regen`, 3I/ATLAS 5/5 `granite_first_pass`, 2I/Borisov 0/5 on a bare `13` from "March 13 2030" written without its comma. Ten of fifteen grounded. The comma-less shape was then recognised so that spelling is rejected whole rather than fragmenting, and the confirming re-run never happened: the watsonx account returned `token_quota_reached` (HTTP 403) partway through, which serves the floor with regens=0 and no findings and reads in a summary table exactly like a gate rejection. The passing answer texts were lost to the overwrite and could not be recaptured. Reported as measured, with the gap named | (runs only) |
...
reading, because it is a record of the gate catching things and of what it
cannot catch.

| Step | Action | Output |
|---|---|---|
| The page served as a file | Claude Code | The committed `index.html`, one self-contained document with its stylesheet, script and two photographs inline, served at `GET /` by `FileResponse`. The 206-line `_page()` that assembled HTML in Python went with it: a template that builds the page in one place and a file that holds it in another are two places for the page to be edited. Served bytes hash-checked against the committed file | `app/main.py`, `web/index.html` |
| Numbers out of the template | Claude Code | The page arrived holding an embedded Borisov answer and the two figures of the gate exhibit, and `test_index_states_no_number` failed on them, correctly. Object answers now fetch `/explain/{key}`; evidence answers fetch `/evidence/{eid}`; the accepted and rejected figures in "See the check run" fetch `/gate/demo/oumuamua`. That last was the sharp one: the section exists to show that no number reaches a reader ungrounded, so a figure typed into it would be the single exception it was built to deny. Ten figures counted out of the template, zero left | `web/index.html`, `tests/test_serve_imports.py` |
| Two shapes from one URL | Claude Code | `/gate/demo/{key}` answers a browser in prose and a caller in json, negotiated by `?format=` then by Accept, so curl and every existing caller are unchanged. The readable view is a committed file that fetches `?format=json` itself, which is why it holds no figure either | `app/main.py`, `web/gate.html` |
| The object was never named | Claude Code | A grounded Borisov generation opened "This trajectory takes a spacecraft from Earth to a target planet", three times over. Every figure came from the manifest and the gate passed it, correctly: it checks numbers and this is a noun. The prompt had never said what the object was. Fixed in the question turn, per object in the generation script and from the manifest header for any caller that supplies no question, so `/explain?live=1` is fixed too. The designation is not a number and never enters the permitted numbers; the tokenizer already refuses a digit fused to a letter, which is what keeps `C3`, `J2000` and `1I` out, and that is now asserted for all three designations including the slash form | `agent/explain.py`, `data/generate_answer.py`, `tests/test_explain.py` |
| A floored run that could not be read | Claude Code | The verification generation floored on three attempts and the script printed the refused tokens and nothing else, so diagnosing it meant reconstructing candidate spellings offline until their findings matched. Every rejection was typography: `km²·s⁻²` and `km·s⁻¹` reached the gate as a wrong-unit on `km^2` and `km` with a loose `2` and `1` beside them, `March 13, 2030` written with U+00A0 left a bare `13`, and `1 728` was a space thousands separator. `verify/normalize.py` gained one closed table of six space characters and three multiplication dots, all mapped to an ASCII space; folding the dot is what makes the rest work, since `km²·s⁻²` becomes `km^2 s^-2`, which the existing table already declared. No unit spelling added, no rendering relaxed, and a fabricated value is the same wrong string in every typography, asserted directly. U+00D7 deliberately absent, and a space between digit groups deliberately not closed up. The script now prints the rejected prose, which paid for itself on the next run | `verify/normalize.py`, `tests/test_normalize.py`, `data/generate_answer.py` |
| A source paper that does not exist | Claude Code | The next generation grounded first pass and closed with "the asymptotic value of 16.93682 km/s reported in the source paper". No published intercept study of 2I/Borisov exists, which is the entire content of its verification status, and all three intercept manifests declare zero published entries. `check_attribution` could not see it: it asks whether a token is labelled as published, and that sentence labels nothing, it states an origin. Widening the pattern was measured first and flags correct comparative prose, where a computed figure is set against a real published one in the same sentence. So the question asked is about the manifest rather than the phrasing: where the solver declared no published entry, no figure has a source to be reported in. The floor is what shaped the pattern, since it says "the quantity the mission-design literature compares against" and a rule that flagged that would make the floor fail its own gate and leave nothing safe to serve. Proven three ways offline before the next call was made | `verify/groundedness.py`, `verify/corpus.py`, `tests/test_groundedness.py` |
| The rule fired live | Claude Code | The re-run reached for the same invented paper on attempt 0, the new rule refused it, the phrase was fed back, and attempt 1 dropped the clause. `granite_after_regen`, grounded. That is the gate being watched catching a fabrication that had been invisible to it an hour earlier | (run only) |
| Feasibility, four times, uncatchable | Claude Code | Every Granite answer of the evening claimed the mission could be flown: "a spacecraft could intercept", "could reach", "could fly from Earth to". HITS models no launch vehicle and cannot make that claim, and the floor says so in as many words. An instruction forbidding it was added to the standing rules in the system turn and contradicted on the next run, and the one after. Four samples across three objects. The gate cannot see it: every number is real and the claim around them is not one the solver made. Not iterated on further; the wording approach is recorded as exhausted | `agent/explain.py`, `tests/test_explain.py` |
| Three floors, shipped as a decision | Claude Code | 1I/'Oumuamua grounded first pass and said "could reach". 3I/ATLAS floored on its own, three attempts each rejected on a spelled-out "twenty", the model repairing the first instance every round and never the one in the closing sentence. 2I/Borisov grounded cleanly and still said "could intercept". So all three ship the deterministic floor, frozen through an explicit `--floor` path rather than by loosening the script's refusal to write a floored run: one stops a floor being written when a Granite answer was wanted, the other writes one when the floor is what was wanted. It reads no credential, constructs no client, is gated before it is written, and records `served_by=deterministic_floor` with an empty `model_id` | `data/generate_answer.py`, `data/answers/` |
| A promise the code had not kept | Claude Code | `app/answers.py` had documented since it was written that `load()` may skip re-checking grounding because CI runs the gate over every committed answer. No such test existed. It cost nothing while no answer was committed and would have cost the guarantee from the moment one was. Written now, and it bites at refreeze rather than at generation: an answer frozen agreeing with its manifest keeps being served, still labelled grounded, after a refreeze makes its figures ones the solver no longer emits. Verified by changing one digit of a committed answer, watching two tests fail, and putting it back | `tests/test_answers.py` |
...
computed figure credited to a source paper that does not exist (caught by review
first, then by a new deterministic rule, then by that rule live); a spelled-out
"twenty"; a middle dot that tore an exponent off its unit.

**What it did not catch, and cannot:** four feasibility claims. The gate proves
every figure came from the solver. It cannot tell whether the sentence around a
figure is true. That asymmetry is the honest summary of what this layer is, and
shipping the floor on all three objects is the response to it rather than a
disappointment.
