...
`plots/validation_comparison.png`, generated by
`solver.plot.plot_validation_comparison_png` from a completed `validate()` run.
The interactive version, `plots/validation_comparison.html`, carries the same
five rows with hover detail and citations; it is 4.6 MB of embedded javascript
and GitHub will not display it, so the static render is the one a reader meets
first and the HTML is kept for the judges page. Both call the same
`_comparison_rows`, so they cannot disagree about a number.
