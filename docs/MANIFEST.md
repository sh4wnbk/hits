...
#### The closed set of date forms

A date entry renders in exactly two forms, generated by
`solver.manifest.date_renderings`: the ISO form, which is canonical, and one
human form.

| `text_value` | renderings |
|---|---|
| `2018-06-07` | `2018-06-07`, `June 7, 2018` |
| `2027-06-21` | `2027-06-21`, `June 21, 2027` |

A date used to render as itself and nothing else, and the prompt carried a rule
forbidding the month-name form. Forbidding the way people write dates in order
to protect a string match is the tail wagging the dog, and it did not work:
Granite wrote "June 7, 2017" anyway, the ISO pattern stopped matching, and the
date fragmented into a bare `7` that the explanation never wrote as a quantity.

Two, and exactly two. `7 June 2017`, `Jun 7 2017` and `07/06/2017` are not
accepted. An open set of date spellings is a fuzzy match wearing a table's
clothes, and the construction assertion enforces the closure: an entry whose
renderings are not exactly `date_renderings(text_value)` fails to build.

#### Recognition is wider than acceptance

The tokenizer recognises every date **shape** it can, while the manifest
accepts two **forms**. These are different sets and the difference is load
bearing.

Matched in pieces, `September 20, 2030` is a `20` and a `2030`. On a manifest
whose flight time is twenty years and whose departure year is 2030, both are
real renderings, so a date the solver never emitted read as one it did. That
was observed live and recorded as a gate limit.

A date is now one token. `September 20, 2030` is looked up whole and either is a
rendering or is not, and `March 20, 2030` is rejected as a single fabricated
token rather than passing as two grounded digits. Shapes the manifest does not
accept (`20 September 2030`, `Sep 20, 2030`) are recognised for exactly this
reason: so they arrive as one unmatched phrase instead of fragmenting into
grounded digits. Widening recognition can only make the gate stricter, never
looser.

The one shape deliberately not recognised is a bare `Month YYYY`. "The June 2027
launch" is ordinary prose about a grounded year, and swallowing it would reject
correct writing.
